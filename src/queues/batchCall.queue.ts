 ...
 * - Completion via post_call_transcription webhooks (manual sync: POST /batch-calling/:jobId/sync)
 */
...
const setupQueueProcessor = () => {
  ...
  batchCallQueue.process('submit-batch-call', 5, async (job: Bull.Job) => {
    ...
    try {
      ...
      console.log(
        `[Batch Call Queue] Batch submitted (${result.id}) — completion via post_call_transcription webhooks`
      );
